...
if ( require.main === module ) {
  commandLine(process.argv.slice(2), process.exit);
}
...
function commandLine(args, console, exitCallback) {
  ...
  var bundleFile = (!parseOnly && getValueArg('bundle')) || undefined;
  var skipWrite = bundleFile || parseOnly;
  var bundleProperty = getValueArg('prop') || undefined;
  ...
  function writeBundle(filePath, bundleName, modules, sandbox) {
    ...
    if ( sandbox ) {
      output.push("var $rt=$i.getRuntime({}),$r=$rt.resolvers(),");
    }
    else {
      output.push("var $rt=$i.getRuntime(),$r=$rt.resolvers(),");
    }
    output.push("$b={},$c={},$d='/index';");
    output.push("$r.push({");
    ...
    output.push("var $rs=$c[$n]=$m.exports();");
    output.push("return $rs;},");  // cache exports and return
    ...
    output.push("})($b,$i.getRuntime({resolvers:$r}));");
