...
function generateTemplateBody(parseOutput, options) {
  options = options || {};
  var functionWrapper = options.functionWrapper || nodeWrapper;
  ...
  function createEvaluator(node) {
    if ( !isArray(node) ) {
      ...
      return globals.literal(lits[node]);
    }
  ...
  function createBindEvaluator(memberNode, argNodes) {
    var bindFunc = globals.runtimeImport('bind');
  ...
  function createCallEvaluator(memberNode, argNodes) {
    var execFunc = globals.runtimeImport('exec');
    var member = createEvaluator(memberNode);
    var args = createArrayEvaluator(argNodes);
  ...
  function createForEvaluator(rangeNodes, statementNodes, elseNodes) {
    var looper = globals.runtimeImport('looper');
  ...
  function createFormatEvaluator(formatNode, exprNode) {
    ...
    var funcId = globals.runtimeImport('buildFormatterCache');
    var id = globals.nextId('b');
    globals.push([id, '=', funcId, '()'].join(''));
  ...
  function createMemberEvaluator(parentNode, elemNode) {
    var getFunc = globals.runtimeImport('getProperty');
