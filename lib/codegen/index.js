...
function generateTemplateBody(parseTree, literals, options) {
  ...
  function createAssignEvaluator(assignmentDefs) {
    var assigns = map(assignmentDefs, function(assignmentDef) {
      return [literals[assign[0]], defer(assign[1])];
    });
    gen.declare(assigns);
  }
  ...
  function createOpenTagEvaluator(nameNode, attributeDefs, selfClose) {
    ...
    var attributes = defer(createDictionaryEvaluator, attributeDefs);
  ...
  function createCloseTagEvaluator(nameNode) {
    gen.statement(function () {
      gen.call(gen.writer('closeTag'), [defer(nameNode)]);
    });
  }
  ...
  function createMatchEvaluator(leftNode, rightNode) {
    ...
    if ( !isArray(rightNode) ) {
      var matcher = globals.builderForLiteral('buildMatcher', right);
  ...
  function createEqEvaluator(leftNode, rightNode) {
    createBinaryEvaluator(leftNode, rightNode, '===');
  }
  ...
  function createArrayEvaluator(arrayNodes) {
    ...
    for ( var i = 0, len = arrayNodes.length; i < len; i++ ) {
      result.push(defer(arrayNodes[i]));
    }
    gen.array(result);
  }
  ...
  function createDictionaryEvaluator(assignmentDefs) {
    var result = map(assignmentDefs, function (assignmentDef) {
      return [defer(assign[0]), defer(assign[1])];
    });
    gen.object(result);
  }
