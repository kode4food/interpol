...
var idCharsLength = idChars.length;

function toIdentifier(value, prefix) {
  var str = '';
  if ( value === 0 ) {
    return prefix + idChars.charAt(0);
  }
  while ( value ) {
    var digit = idChars.charAt(value % idCharsLength);
    value = Math.floor(value / idCharsLength);
    str = digit + str;
  }
  return prefix + str;
}
...
function createGlobals() {
  ...
  function nextId(prefix) {
    ...
    var id = toIdentifier(next, prefix);
