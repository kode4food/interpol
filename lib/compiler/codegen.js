...
function generateModuleBody(strippedTree, literals, options) {
  ...
  // generate an evaluator to deal with 'export' statements
  function createExportEvaluator(exportedNodes) {
    // the context to which modules are exported, must be the root context
    var exporter = globals.runtimeImport('exporter');
