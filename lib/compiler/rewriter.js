...
function rewriteSyntaxTree(syntaxTree, warnings) {
  ...
  function rewrite(node) {
    ...
    if ( hasOperator(node) ) {
      ...
      node = annotationSelfReferences(node);
    }
    return node;
  }
  ...
  function hoistPartials(statements) {
    ...
    if ( invalid ) {
      if ( others.length ) {
        warnings.push({
          line: invalid[0].line,
          column: invalid[0].column,
          message: "Will only perform 'hoisting' if all partials are " +
                   "placed after other statements"
        });
      }
      return statements;
    }
  ...
  function annotationSelfReferences(node) {
    return node;
  }
}
