...
paramDef
  = using:( Using _ { return true; })? param:(Identifier / expr)  {
      return [param, using];
    }
...
usingStatement
  = op:Using _ exprs:elemList statements:statementsTail  {
      return [sym(op), exprs, statements];
    }

forStatement
  = op:For _ ranges:ranges _ ":" __ statement:statement  {
...
ifStatement
  = op:IfUnless _ expr:expr _ ":" __ statement:statement  {
      if ( !op ) {
