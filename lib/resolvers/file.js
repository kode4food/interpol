 ...
 */
function createFileResolver(interpol, options) {
  var cache = {};
   ...
   */
  function reloadIfNeeded(name, runtime) {
    ...
    if ( statusResult.dirty ) {
      try {
        var template = resolveTemplate(sourcePath, runtime);
  ...
  function resolveMonitoredModule(name, runtime) {
    // Always attempt to reload the file if necessary
    var result = reloadIfNeeded(name, runtime);
    return result.module;
  }

  function resolveCachedModule(name, runtime) {
    ...
    result = reloadIfNeeded(name, runtime);
    return result.module;
  }

  function resolveMonitoredExports(name, runtime) {
  ...
  function resolveCachedExports(name, runtime) {
  ...
  function resolveJS(sourcePath, runtime) {
    ...
    var wrapper = context.module.exports;
    var createTemplate = wrapper.createTemplate;
    if ( !wrapper.__intNodeModule || typeof createTemplate !== 'function' ) {
      throw new Error("Module is not an Interpol Template: " + sourcePath);
    }

    return createTemplate(runtime);
  }
}
...
function registerResolver(interpol) {
  interpol.createFileResolver = registeredCreateFileResolver;

  function registeredCreateFileResolver(options) {
    return createFileResolver(interpol, options);
  }
}
