...
function createFileResolver(interpol, options) {
  ...
  var isDirty = options.monitor ? createDirtyChecker() : notDirty;
  ...
  function reloadIfNeeded(name, options) {
    ...
    var loadable = resolveLoadable(sourcePath);
    ...
    if ( loadable ) {
      try {
        var template = resolveTemplate(sourcePath, options);
        var cached = cache[name] = { module: template };
        return cached;
      }
      catch ( err ) {
  ...
  function resolveInt(sourcePath, options) {
    var content = fs.readFileSync(sourcePath).toString();
    return interpol(content, options);
  }

  function resolveJS(sourcePath, options) {
    var content = fs.readFileSync(sourcePath).toString();
    ...
    var runtime = options;
    if ( !util.isInterpolRuntime(runtime) ) {
