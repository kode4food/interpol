  ...
  function createPartialEvaluator(nameLiteral, paramDefs,
    ...
    function closureEvaluator(ctx /*, writer */) {
      ...
      if ( !ctx.hasOwnProperty(name) || !isInterpolPartial(ctx[name]) ) {
        bodyEvaluator = guard ? guardedBodyEvaluator : unguardedBodyEvaluator;
      }
      else {
        oldEvaluator = ctx[name].__interpolBodyEvaluator;
        bodyEvaluator = branchedBodyEvaluator;
        newEvaluator = guard ? guardedBodyEvaluator : unguardedBodyEvaluator;
      }
      ...
      function branchedBodyEvaluator(ctx, writer) {
        if ( !oldEvaluator(ctx, writer) ) {
          return newEvaluator(ctx, writer);
        }
        return true;
      }
    }
