...
function compileModule(template) {
  ...
  function promoteRawLiteralOutput(statements) {
    ...
    function matcher(statement) {
      return isStatementOperator(statement, 'ou') &&
  ...
  function convertLiteralHTML(statements) {
    ...
    function matcher(statement) {
      return isStatementOperator(statement, ['op', 'cl', 'ct']);
    }

    function processStatement(statement) {
      var op = isStatementOperator(statement, ['op', 'cl', 'ct'])
  ...
  function rollupForLoops(statements) {
    ...
    function matcher(statement) {
      return isStatementOperator(statement, 'fr') &&        // is a for loop
             statement[2].length === 1 &&                   // one child
             isStatementOperator(statement[2][0], 'fr') &&  // nested for loop
...
function matchStatement(op) {
  ...
  function matcher(statement) {
    return isStatementOperator(statement, op);
  }
}
