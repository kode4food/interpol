...
function generateFunctionBody(parseOutput) {
  ...
  buffer.push(createStatementsEvaluator(parseOutput.n));
  return buffer.join('');
  ...
  function generateRuntimeImport(funcName) {
    ...
    buffer.push("var " + id + " = runtime." + funcName + ";");
    return id;
  }
  ...
  function generateBuilder(funcName, literalValue) {
    ...
    buffer.push("var " + id + " = " + funcId + "(" + literalId + ");");
    return id;
  }
  ...
  function createBinaryEvaluator(left, right, operator) {
  ...
  function createStatementsEvaluator(statementNodes) {
    var statements = createArrayEvaluator(statementNodes).reverse();
    var slen;
    var statement;
  ...
  function createOrEvaluator(leftNode, rightNode) {
    ...
    return createBinaryEvaluator($1, $2, '||');
  }
  ...
  function createFormatEvaluator(formatNode, exprNode) {
    ...
    if ( !isArray(formatNode) ) {
      var builder = generateBuilder('buildFormatter', $1);
      return builder + '(' + createEvaluator(exprNode) + ')';
    }

    // Generate a global template cache for dynamic evaluations
    var funcId = generateRuntimeImport('buildFormatterCache');
    var id = generatedBuilders[key] = nextId('$b');
    buffer.push("var " + id + " = " + funcId + "();");
