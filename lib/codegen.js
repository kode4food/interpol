...
function generateFunctionBody(parseOutput) {
  ...
  function createImportEvaluator(fromNodes) {
    ...
    for ( var i = 0, ilen = fromNodes.length; i < ilen; i++ ) {
      ...
      if ( !toResolve ) {
        var lcl = generateLocalForName(moduleName);
        var nameLit = generateLiteral(moduleAlias);
        result.push('var ' + lcl + '=c[' + nameLit + ']=' + importer + '()');
        continue;
      }
      ...
      for ( var j = toResolve.length - 1; j >= 0; j-- ) {
        ...
        var name = generateLocalForName(aliasName);
        var nameLit = generateLiteral(aliasName);
        var propLit = generateLiteral(aliasProperty);
        vars.push(name + '=c[' + nameLit + ']=' + anon + '[' + propLit + ']');
      }
  ...
  function createPartialEvaluator(nameLiteral, paramDefs,
    ...
    var definePartial = gene
    var name = lits[nameLiteral];
              var name = generateLocalForName(aliasMap[0]);

    var guard = guardNode && createEvaluator(guardNode);var params = [null].conc
    at(expandLiterals(paramDefs));
    var plen = params.length;
    var statements = createStatementsEvaluator(statementNodes)

              var name = generateLocalForName(aliasMap[0]);

    var guard = guardNode && createEvaluator(guardNode);return guard ? guardedClosureEvaluator : unguardedClosureE
    valuator;
